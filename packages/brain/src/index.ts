...
import { BrainResponseType, env, logger, LocalAnalysis } from '@discord-agent/commons';
...
export class Brain {
    public async processMessage(message: {
        ...
        if (resultCache.has(cacheKey)) {
            ...
            return this.handleBrainResponse(resultCache.get(cacheKey)!, message.guild.id);
        }

        // 1. Triage for commands
        if (message.content.startsWith('!') || message.content.startsWith('/')) {
            // Simple command triage to avoid LLM call
            logger.info(`[Triage] Message '${message.content}' identified as command.`);
        ...
        const memories = await memoryManager.retrieve(message.content, message.guild.id);
        const memorySnippets = memories.map(m => m.metadata.summary || m.metadata.text);

        // 4. Construct the prompt
        const systemPrompt = constructSystemPrompt(
            persona,
            { name: message.guild.name, channelName: message.channel.name, allowedCommands: [] },
        ...
        try {
            ...
            await this.handleBrainResponse(brainResponse, message.guild.id);

        } catch (error) {
            logger.error({ err: error }, 'Error during Gemini API call or subsequent processing.');
    ...
    private async getLocalAnalysis(text: string): Promise<LocalAnalysis> {
        try {
            const response = await axios.post(`${env.LOCAL_LLM_API_URL}/analyze`, { text });
            return response.data;
        } catch (error) {
            logger.error({ err: error }, 'Failed to get local analysis.');
            // Return a neutral default
            return { sentiment: 'neutral', sentiment_score: 0, toxicity: 0, emotion: 'unknown', intent: 'unknown' };
        }
    }
    ...
    private async handleBrainResponse(response: BrainResponseType, guildId: string) {
        // Execute tasks via MCP
        if (response.tasks && response.tasks.length > 0) {
            logger.info({ tasks: response.tasks }, 'Forwarding tasks to MCP.');
            await mcpApi.post('/execute', { tasks: response.tasks });
        }
        ...
        if (response.memory_ops && response.memory_ops.length > 0) {
            for (const op of response.memory_ops) {
                switch (op.op) {
                    case 'upsert':
                        // The LLM can't generate a vector, so we create it now.
                        // The LLM's job is to decide WHAT to remember.
                        if (op.meta?.text) {
                           await memoryManager.upsert({
                               text: op.meta.text,
                               summary: op.meta.summary || op.meta.text, // LLM should provide a summary
                               guildId: guildId,
        ...
        // The actual sending of the response message is handled by the `bot` app
        // This function's return value (or an event) will trigger it.
        // For now, we just log it.
        if (response.should_respond && response.response) {
            logger.info({ response: response.response }, 'Brain decided to respond.');
            // This is where you'd send the response back to the bot app to be delivered to Discord.
        }
    }
}
